...
x1_s6 = np.array([1, 2, 2, 4, 5])
x2_s6 = np.array([5, 8, 3, 1, 2])
...
def neuron_model(w0, w1, w2):
    ...
    plt.scatter(x1_s6, x2_s6, c=c, zorder=2, s=120, edgecolors='k')
    plt.show()
    
def table(w0, w1, w2):
    z = w0 + w1*x1_s6 + w2*x2_s6
    p = 1 / (1 + np.exp(-z))
    pi = np.where(c == 'b', p, 1 - p)
    lik = np.product(pi)
    
    df = pd.DataFrame({'x1':x1_s6, 'x2':x2_s6, 'col':c, 
